...
// CHECK REQUIRED VALUE AND POPULATE AUTHOR MODEL
    let unknownProperties = Object.keys(properties).filter((key) => {return !(key in authorProperties)})
    let missingProperties = [];

	for (let property in authorProperties) {
        if (authorProperties[property].value == undefined && authorProperties[property].required) {
